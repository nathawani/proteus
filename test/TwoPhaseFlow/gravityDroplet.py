...
class clsvof_init_cond(object):
    def uOfXT(self,X,t):
        ...
        y=X[1]
        if flat_inlet:
            if x-L[0]/2. >= -h0 and x-L[0]/2. <= h0:
                if y==L[1]:
                    return 0.
                else:
                    return L[1] - y
            else:
                return min(np.sqrt( (x-(L[0]/2.-h0))**2 + (y-L[1])**2),
                           np.sqrt( (x-(L[0]/2.+h0))**2 + (y-L[1])**2) )
        else: #circular inlet
            rInflow = np.sqrt((x-L[0]/2.)**2 + (y-L[1])**2)
            dB = -(h0-rInflow)
            return dB
...
def pressure_DBC(x,flag):
    if flag == boundaryTags['top'] and not (x[0]>=L[0]/2.-h0 and x[0]<=L[0]/2.+h0):
...
def vel_u_DBC(x,flag):
    if flag==boundaryTags['bottom'] or flag==boundaryTags['left'] or flag==boundaryTags['right']:
        return lambda x,t: 0.0
    elif flag == boundaryTags['top'] and (x[0]>=L[0]/2.-h0 and x[0]<=L[0]/2.+h0):
        return lambda x,t: 0.0
...
def vel_v_DBC(x,flag):
    if flag==boundaryTags['bottom'] or flag==boundaryTags['left'] or flag==boundaryTags['right']:
        ...
        return lambda x,t: -u0
    #return lambda x,t: 100*(x[0]-0.45)*(x[0]-0.55)*1/(0.55*0.45)

# ADVECTIVE FLUX #
def pressure_AFBC(x,flag):
    if not(flag == boundaryTags['top'] and not (x[0]>=L[0]/2.-h0 and x[0]<=L[0]/2.+h0)):
        return lambda x,t: 0.0
...
def clsvof_DBC(x,flag):
    if flag == boundaryTags['top']:
        if x[0]>=L[0]/2.-h0 and x[0]<=L[0]/2.+h0:
...
myTpFlowProblem.SystemPhysics.rho_0 = 1223.0
myTpFlowProblem.SystemPhysics.nu_0 = 9.2E-5
myTpFlowProblem.SystemPhysics.rho_1 = 1.2
myTpFlowProblem.SystemPhysics.nu_1 = 1.5E-5
myTpFlowProblem.SystemPhysics.surf_tension_coeff = 0.066
